    ...
    def __init__(self,root):
        ...
        def addData():
            if studentID.get()=="" or Firstname.get()=="" or Surname.get()=="":
                ...
                cur =sqlCon.cursor()
                cur.execute("insert into tseccanteendb values(%s,%s,%s,%s,%s,%s)",(studentID.get(),Firstname.get(),Surname.get(),Address.get(),Gender.get(),Mobile.get()))
                sqlCon.commit()
                sqlCon.close()
        ...
        def update():
                sqlCon=pymysql.connect(host="localhost",user="root",password="7777",database="tseccanteendb")
                cur =sqlCon.cursor()
                cur.execute("update tseccanteendb set firstname=%s,surname=%s,address=%s,gender=%s,mobile=%s where studentID=%s", (
                Firstname.get(),
                Surname.get(),
                Address.get(),
                Gender.get(),
        ...
        self.cboGender['values']=(' ','dal_pakwan','veg thali','samosa','chai','koki','masala toast','misal pav','dosa','pizza','burger','coffee','uttapa','pav bhaji','fried rice','noodels','nachos','dhokla','mango juice','pastry','gulab jamun')
        ...
        self.student_records=ttk.Treeview(LeftFrame, height=14, columns=("rollno","name","branch","division","menu","quantity"), yscrollcommand=scroll_y.set)
